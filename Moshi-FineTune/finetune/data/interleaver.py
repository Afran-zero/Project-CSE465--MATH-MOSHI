    ...
    def __call__(self, wav: np.ndarray, start_sec: float, path: str) -> Sample:
        with torch.no_grad():
            ...
            with open(info_file) as f:
                data = json.load(f)
                # Safe access to alignments
                alignments = data.get("alignments", None)

            if alignments is None:
                # Handle case where 'alignments' are missing
                print(f"Warning: 'alignments' missing for {path}. Proceeding without them.")
                alignments = []  # Empty list or handle accordingly
